...
def responsible_for_conducting(df, excel_file_path):
...
def responsible_for_rank(df, excel_file_path):
    df = df[(df['Кол. участ.'] < 300)]
...
def responsible_for_rf(df, excel_file_path):
...
def responsible_for_contact(df, excel_file_path):
    df = df[(df['Ответственные лица'].str.contains(
...
def responsible_main_concat(df, excel_file_path):
    responsible_for_conducting(df, excel_file_path)
    responsible_for_rank(df, excel_file_path)
    responsible_for_rf(df, excel_file_path)
